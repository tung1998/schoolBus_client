//THÊM
function createCOMMON(COMMON, accessToken = '', callback) {
    Meteor.call('COMMON.create', COMMON, accessToken, callback)
}

//XEM HẾT
function getCOMMON(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('COMMON.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getCOMMONsByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('COMMON.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getCOMMONByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('COMMON.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateCOMMON(COMMON, accessToken = '', callback) {
    Meteor.call('COMMON.update', COMMON, accessToken, callback)
}

//XÓA
function deleteCOMMON(COMMONID, accessToken = '', callback) {
    Meteor.call('COMMON.delete', COMMONID, accessToken, callback)
}
//****************************************************
