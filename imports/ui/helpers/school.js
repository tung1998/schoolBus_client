//THÊM
function createSchool(School, accessToken = '', callback) {
    Meteor.call('School.create', School, accessToken, callback)
}

//XEM HẾT
function getSchool(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('School.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getSchoolsByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('School.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getSchoolByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('School.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateSchool(School, accessToken = '', callback) {
    Meteor.call('School.update', School, accessToken, callback)
}

//XÓA
function deleteSchool(SchoolID, accessToken = '', callback) {
    Meteor.call('School.delete', SchoolID, accessToken, callback)
}
//****************************************************
