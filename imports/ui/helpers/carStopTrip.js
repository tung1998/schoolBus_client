//THÊM
function createCarStopTrip(CarStopTrip, accessToken = '', callback) {
    Meteor.call('CarStopTrip.create', CarStopTrip, accessToken, callback)
}

//XEM HẾT
function getCarStopTrip(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('CarStopTrip.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getCarStopTripsByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('CarStopTrip.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getCarStopTripByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('CarStopTrip.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateCarStopTrip(CarStopTrip, accessToken = '', callback) {
    Meteor.call('CarStopTrip.update', CarStopTrip, accessToken, callback)
}

//XÓA
function deleteCarStopTrip(CarStopTripID, accessToken = '', callback) {
    Meteor.call('CarStopTrip.delete', CarStopTripID, accessToken, callback)
}
//****************************************************
