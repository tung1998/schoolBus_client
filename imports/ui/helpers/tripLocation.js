//THÊM
function createTripLocation(TripLocation, accessToken = '', callback) {
    Meteor.call('TripLocation.create', TripLocation, accessToken, callback)
}

//XEM HẾT
function getTripLocation(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('TripLocation.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getTripLocationsByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('TripLocation.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getTripLocationByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('TripLocation.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateTripLocation(TripLocation, accessToken = '', callback) {
    Meteor.call('TripLocation.update', TripLocation, accessToken, callback)
}

//XÓA
function deleteTripLocation(TripLocationID, accessToken = '', callback) {
    Meteor.call('TripLocation.delete', TripLocationID, accessToken, callback)
}
//****************************************************
