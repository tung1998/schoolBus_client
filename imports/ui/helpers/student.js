//THÊM
function createStudent(Student, accessToken = '', callback) {
    Meteor.call('Student.create', Student, accessToken, callback)
}

//XEM HẾT
function getStudent(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('Student.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getStudentsByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('Student.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getStudentByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('Student.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateStudent(Student, accessToken = '', callback) {
    Meteor.call('Student.update', Student, accessToken, callback)
}

//XÓA
function deleteStudent(StudentID, accessToken = '', callback) {
    Meteor.call('Student.delete', StudentID, accessToken, callback)
}
//****************************************************
