//THÊM
function createNanny(Nanny, accessToken = '', callback) {
    Meteor.call('Nanny.create', Nanny, accessToken, callback)
}

//XEM HẾT
function getNanny(accessToken = '', { driverBlockedType = 'all' } = {}) {
    return new Promise((resolve, reject) => {
        Meteor.call('Nanny.getAll', accessToken, { driverBlockedType }, (err, result) => {
...
function getNannysByPage(accessToken = '', page = 1) {
    return new Promise((resolve, reject) => {
        Meteor.call('Nanny.getByPage', accessToken, page, (err, result) => {
            resolve(result);
        });
    });
}
...
//XEM THEO ID
function getNannyByID(accessToken = '') {
    return new Promise((resolve, reject) => {
        Meteor.call('Nanny.getByID', accessToken, (err, result) => {
            resolve(result);
        });
    });
}
...
//UPDATE
function updateNanny(Nanny, accessToken = '', callback) {
    Meteor.call('Nanny.update', Nanny, accessToken, callback)
}

//XÓA
function deleteNanny(NannyID, accessToken = '', callback) {
    Meteor.call('Nanny.delete', NannyID, accessToken, callback)
}
//****************************************************
