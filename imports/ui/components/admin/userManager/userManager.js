...
function submitBlockUser() {
    if ($('#reason-block').val()) {
        ...
        MeteorCall(_METHODS.user.BlockUser, data, accessToken).then(result => {
...
function clickUnBlockUser(e) {
    ...
    handleConfirm("Bạn muốn mở khóa tài khoản này").then(result => {
        if (result.value) {
            MeteorCall(_METHODS.user.UnblockUser, data, accessToken).then(result => {
                Swal.fire({
                    icon: "success",
...
function reloadTable(page = 1, limitDocPerPage = LIMIT_DOCUMENT_PAGE, options = null) {
    ...
    MeteorCall(_METHODS.user.GetByPage, {
        page: page,
